...
const noteToFreq = (noteName: string): number => {
  ...
  // Normalize pitch for mapping: "Bb" -> "BB", "Eb" -> "EB"
...
const PHRASES: LoopItem[] = [
  { 
    ...
    notes: [
      { freq: "BB5", position: getNotePosition("BB5"), offset: 11.645 }
    ] 
  },
  { 
  ...
  { 
    ...
    notes: [
      { freq: "BB3", position: getNotePosition("BB3"), offset: 5.696 }
    ] 
  },
  { 
...
export const GenerativeMusic: React.FC = () => {
  ...
  /**
   * playNote: Piano synthesis using robust setTargetAtTime envelopes
   */
  const playNote = (freq: number, volume = 0.5) => {
    const ctx = audioCtxRef.current;
    if (!ctx || !masterGainRef.current) return;
    
    // Explicitly resume to avoid browser auto-pause issues
    if (ctx.state === 'suspended') ctx.resume();

    const now = ctx.currentTime;
    
    // Scaling decay by frequency: higher notes decay faster
    const decayBase = Math.max(0.1, 1.2 - (Math.log2(freq / 110) * 0.2));

    const createPianoVoice = (f: number, v: number, d: number, type: OscillatorType = 'sine') => {
      ...
      // setTargetAtTime is standard for safe, non-popping envelopes
      ...
      osc.start(now);
      // Stop long after to allow tail to die out
      osc.stop(now + d * 10 + 0.5);
    };

    // Piano composite: fundamental + harmonics
    createPianoVoice(freq, 0.9, decayBase, 'triangle'); 
    createPianoVoice(freq * 2, 0.4, decayBase * 0.7);    
    createPianoVoice(freq * 3, 0.2, decayBase * 0.5);    
    createPianoVoice(freq * 4, 0.1, decayBase * 0.3);    
    
    // Hammer strike transient
    const hammer = ctx.createOscillator();
    const hammerGain = ctx.createGain();
    hammer.frequency.setValueAtTime(freq * 12, now);
    hammerGain.gain.setValueAtTime(0, now);
    hammerGain.gain.setTargetAtTime(volume * 0.4, now, 0.002);
    hammerGain.gain.setTargetAtTime(0, now + 0.005, 0.01);
  ...
  const resetAndAutoInit = async () => {
    ...
    // Spread the start times slightly for a more organic initial entry
    PHRASES.forEach((p, i) => {
          ...
          <div className="flex flex-col items-center gap-12 pt-16 border-t border-zinc-100">
            <div className="flex flex-wrap justify-center items-center gap-10">
              ...
              <div className="flex items-center gap-6 px-10 py-4 bg-zinc-50 rounded-full border border-zinc-100">
                ...
                <button 
                  onClick={() => playNote(noteToFreq("BB5"), 0.55)}
            ...
            <div className="text-center space-y-2">
              ...
              {autoMode && (
                <p className="text-[9px] uppercase tracking-[0.3em] text-emerald-600 font-medium">
                  Status: Full Phasing Simulation Active
                </p>
              )}
            </div>
          </div>
